...
-**관계형 데이터 모델** (Reloational 데이터 테이블로 조직화)
...
## 제약조건
1. Primary Key (PK)
- 각행을 유일하게 식별할수 있는 열(열의 조합)
- Not Nulll과 Unique 의 속성을 갖는다 (개체 무결성)
...
1. 숫자형 데이터 타입
    ...
    -DECIMAL(M,N): 고정 소수점 숫자
        -M 총자릿수, N 소수점 자리수
    -FLOAT(4byte),DOUBLE(8byte) : 부동소수점 숫자
2. 문자열 데이터 타입
3. 날짜/시간 데이터 타입
4. 이진 데이터 타입
5. 논리 데이터 타입
