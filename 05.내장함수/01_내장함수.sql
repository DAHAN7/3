...
-- instr(문자열,찾을문자열) :찾을 문자열이 처음 나타나는 위치 반환
...
-- round(숫자,소수점자리수) : 반올림
SELECT round(10.4567),round(10.45687,2),round(4567.1234,-2);
...
-- str_to_date(날짜타입의 문자열,날짜타입): 문자열을 날짜타입으로 변환
SELECT str_to_date('2024년 6월 19일','%Y년 %m월 %d일');

-- format(숫자,소수점 자리수):숫자를 1000단위마다 콤마가 붙여진 문자열로 반환
SELECT format(1234567812345,2);
SELECT format(1234567812345,0);


-- 5. 집계함수 : count,sum,avg,min,max->Groupby에서 다룸
...
-- ifnull(표현식,대체할 값) : 만약 첫번재 인자 표현식이 null이면 대체값(두번재 인자) 반환 
SELECT ifnull(null,'빈값');
SELECT ename,sal,comm from emp;			 	-- null 존재 컬럼(comm)
SELECT ename,sal,ifnull(comm,0) from emp;	-- null -> 0 대체

-- nullif(표현식1,표현식2): 두 표현식이 같으면 null, 다르면 표현식1(첫번재 인자) 반환
SELECT ifnull(100,100),nullif(100,0);
