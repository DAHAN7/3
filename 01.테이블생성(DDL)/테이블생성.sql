-- 데이터베이스 생성
-- IF NOT EXISTS : 존재하지 않을대만 생성(오류 회피)
...
INSERT INTO `test_db`.`employees` (`employee_id`, 
`first_name`, `last_name`) VALUES ('가나다', '가', '나');
-- 데이터 타입(도메인 제약 조건)에 맞는 데이터를 입력하면 정상 수행
INSERT INTO `test_db`.`employees` (`employee_id`, 
`first_name`, `last_name`) VALUES ('1', '길동', '홍');
...
CREATE TABLE player (
    ...
    DESC player;
    SELECT * FROM player;
    -- 복합 기본키의 각 컬럼은 종복되어도, 두 기본키의 조합이 중복되지 않으면
