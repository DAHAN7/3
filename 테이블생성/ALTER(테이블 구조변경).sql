...
DESCRIBE employees_abc;		 -- 테이블 구조 확인
SELECT * FROM employees_abc; -- 데이터는 복사되지 않음
...
-- 2. 외래키로 사용할 컴럼을 추가(참조할 테이블의 기본키와 같은 타입)
ALTER TABLE employees
	ADD COLUMN department_id INT;

-- 3. 외래키 제약조건 추가(CONSTRAINT 제약조건의 이름)
ALTER TABLE employees
